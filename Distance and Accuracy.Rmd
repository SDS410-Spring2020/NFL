...
Steps:
1) Mutate the new indicator variables 
2) Clean up the distance variables 
3) Join the dfs together 


```{r}

data$PlayDescription<-str_to_lower(data$PlayDescription)
plays<-games_w_coordinates %>% 
...
passes<-plays %>% 
  group_by(Home_long, Home_lat, Visit_long, Visit_lat, PlayDescription, OffensePlayResult, PossessionTeam, PlayResult, GameKey, Season) %>% 
