...
visitor_icons <- awesomeIcons(
  icon = 'ios-close',
  iconColor = 'black',
  library = 'ion',
  markerColor = "orange")

home_icons <- awesomeIcons(
  markerColor = "red")

# Lines_from_arizona_2007 <- data.frame(group = c("Home_Team", "Visit_Team"),
#                     lat = c(mapping_Arizona_Cardinals_2007$Visit_lat, mapping_Arizona_Cardinals_2007$Home_lat),
#                     long = c(mapping_Arizona_Cardinals_2007$Visit_long, mapping_Arizona_Cardinals_2007$Home_long))

mapping_2007 <- mapping_Arizona_Cardinals_2007 %>% 
  leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  setView(-93.65, 42.0285, zoom = 4) %>%
  addMarkers(~Visit_long, ~Visit_lat, icon=visitor_icons) %>%
  addMarkers(~Home_long, ~Home_lat, icon=home_icons) %>%
  addMarkers(
