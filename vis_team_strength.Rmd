...
ep_blt_vs_pit <- full_with_elo %>% 
  filter(home_club_code %in% c("BLT","PIT"),
         ...
         season.y == "2010") %>%
  select(game_key, date, season.y, game_clock, possession_team, ep) 
```
...
```{r}
pit_home_elo <- ggplot(data = pit_full_home, aes(x = date, y = elo1_pre)) +
  geom_line() + 
...
```{r}
pit_visitor_elo <- ggplot(data = pit_full_visitor, aes(x = date, y = elo2_pre)) +
  geom_line() + 
  geom_point(aes(color = is_thurs), size = 2, alpha = 0.6) +
  theme_bw() +
...
```{r}
thurs_count <- full_with_elo %>% 
  select(game_date, home_club_code, game_day, season.x, elo_prob1) %>% 
  distinct(game_date, .keep_all = TRUE) %>% 
  group_by(home_club_code, season.x, game_day) %>% 
  tally() %>% 
  arrange(home_club_code,season.x)
```
